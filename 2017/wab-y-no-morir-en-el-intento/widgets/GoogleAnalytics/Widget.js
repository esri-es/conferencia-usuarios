define([
  ...
  var clazz = declare([BaseWidget], {
    ...
    startup: function() {
      ...
      // this.own(topic.subscribe('onOpen', lang.hitch(this, function(){
      //   console.log()
      // })));
      ...
      this.oldCloseWidget = window._widgetManager.closeWidget;
      // this.oldActivateWidget = window._widgetManager.activateWidget;
      // this.oldMaximizeWidget = window._widgetManager.maximizeWidget;
      // minimizeWidget
      // normalizeWidget
      window.eventsLog = window.eventsLog || [];

      var that = this;

      window._widgetManager.openWidget = function(widget){
        var evLog = window.eventsLog;
        var lastEv = evLog[evLog.length-1];

        if(widget.inPanel.name !== "GoogleAnalytics"){
          var ev = {
            hitType: 'event',
            eventCategory: 'Widget Interaction',
            eventAction: 'openWidget',
            eventLabel: widget.name
          };

          if(!lastEv || JSON.stringify(lastEv) !== JSON.stringify(ev) ){
            window.eventsLog.push(ev)
            console.log("add ",ev);
            ga('send', 'event', ev.eventCategory, ev.eventAction, ev.eventLabel);
          }
        }
      ...
      window._widgetManager.closeWidget = function(widget){
        var evLog = window.eventsLog;
        var lastEv = evLog[evLog.length-1];
        if(widget.inPanel.name !== "GoogleAnalytics"){
          var ev = {
        ...
        that.oldCloseWidget.apply(this, arguments);
        lang.hitch(this, that.oldCloseWidget);
      };


      // window._widgetManager.activateWidget = function(widget){
      //   var evLog = window.eventsLog;
      //   var lastEv = evLog[evLog.length-1];
      //
      //     var ev = {label: "activateWidget", name: widget.name};
      //
      //     if(!lastEv ||  JSON.stringify(lastEv) !== JSON.stringify(ev) ){
      //       window.eventsLog.push(ev)
      ...
      //     }
      //
      //   that.oldActivateWidget.apply(this, arguments);
      //   lang.hitch(this, that.oldActivateWidget);
      // };
      //
      //
      // window._widgetManager.maximizeWidget = function(widget){
      //   var evLog = window.eventsLog;
      //   var lastEv = evLog[evLog.length-1];
      //
      //     var ev = {label: "maximizeWidget", name: widget.name};
      //
      //     if(!lastEv ||  JSON.stringify(lastEv) !== JSON.stringify(ev) ){
      //       window.eventsLog.push(ev)
      //       console.log("maximize ",ev);
      //
      //   }
      //   that.oldMaximizeWidget.apply(this, arguments);
      //   lang.hitch(this, that.oldMaximizeWidget);
      // };
